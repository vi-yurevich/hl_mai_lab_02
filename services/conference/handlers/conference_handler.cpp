                       ...
                       HTTPServerResponse &response)
    {
        ...
        try
        {
                    ...
                    && hasSubstr(request.getURI(), "/conference/lectures"))
            {
                ...
                auto result = database::Conference::find_all_lectures_of_conference_by_conference_id(id);
                if (result)
                {
                ...
                else
                {
                    ...
                    && (request.getMethod() == Poco::Net::HTTPRequest::HTTP_GET)
                    && hasSubstr(request.getURI(), "/conference/authors"))
            {
                long id = atol(form.get("conference_id").c_str());

                auto result = database::Conference::find_all_authors_of_conference_by_conference_id(id);
                if (result)
                {
                    response.setStatus(Poco::Net::HTTPResponse::HTTP_OK);
                    response.setChunkedTransferEncoding(true);
                    ...
                    std::ostream &ostr = response.send();
                    Poco::JSON::Stringifier::stringify(
                        database::User::vectorToJSON_and_hide_password(result.value()), 
                        ostr);
                    return;
                }
                else
